"""
Clinical Batch Processing DAG
Processes one numeric ID range [lower_limit, upper_limit].
"""
...
# ---------- Python callables (no @task, no weird signatures) ----------
...
def _validate_and_get_params(**kwargs):
    ...
    # PythonOperator will push this dict to XCom
    return params
...
def _process_rows(**kwargs):
    ...
    for uid, txt in rows:
        text = txt or ""
        summary = text[:200]
        flag = 1 if "pain" in text.lower() else 0
        processed.append((uid, summary, flag))
...
def _write_results_atomic(**kwargs):
    ...
    try:
        ...
        if processed:
            upsert_sql = f"""
                INSERT INTO {params['sql_output_table']} (unique_id, summary, flag)
                VALUES (%s, %s, %s)
        ...
        conn.commit()
        log.info("Wrote %d rows; audit_id=%s", len(processed), audit_id)
    except Exception:
        conn.rollback()
        cur.execute(
